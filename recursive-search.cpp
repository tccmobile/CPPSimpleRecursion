...
int FindMatch(vector<string> stringsList, string itemMatch, int lowVal, int highVal) {
   ...
   if (itemMatch == stringsList.at(static_cast<vector<string>::size_type>(midVal))) {   // Base case 1: item found at midVal position
      itemPos = midVal;
   }
      ...
      if (itemMatch < stringsList.at(static_cast<vector<string>::size_type>(midVal))) { // Search lower half, recursive call
